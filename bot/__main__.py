...
from bot.model.features_build import build_features_from_bars, features_to_vector
...
def cmd_backfill(args: argparse.Namespace, settings: Settings) -> None:
    symbols = [s.strip().upper() for s in (args.symbols or ",".join(settings.watchlist_symbols)).split(",") if s.strip()]
    data = AlpacaDataClient(settings.alpaca_api_key, settings.alpaca_api_secret, paper=not settings.is_live)
    for sym in symbols:
        log.info("Backfilling %s (%d days)...", sym, args.days)
        df = pd.DataFrame()
        if data.available:
            try:
                df = data.get_minute_bars(sym, days=args.days)
            except Exception as exc:  # noqa: BLE001
                log.warning("Alpaca minute bars failed for %s: %s", sym, exc)
        if df.empty:
            log.info("Falling back to yfinance daily for %s (scan only)", sym)
            daily = fetch_daily(sym, days=args.days)
            if not daily.empty:
                save_bars(settings.data_dir, sym, daily, timeframe="1Day")
                log.info("Saved %d daily bars for %s", len(daily), sym)
            continue
        path = save_bars(settings.data_dir, sym, df)
...
def _sample_training_rows(
    ...
    *,
    ...
    rows: list[dict] = []
    symbols = list_symbols_with_bars(settings.data_dir, "1Min")
    if not symbols:
        # Also try any parquet
        symbols = list_symbols_with_bars(settings.data_dir, "1Day")
    for sym in symbols:
        bars = load_bars(settings.data_dir, sym, "1Min")
        if bars.empty:
            bars = load_bars(settings.data_dir, sym, "1Day")
        if bars.empty or len(bars) < horizon + 30:
            continue
        if "timestamp" in bars.columns:
            mask = rth_mask(bars["timestamp"])
            # Keep indices that are RTH
            rth_indices = np.where(mask.to_numpy())[0]
        else:
            rth_indices = np.arange(len(bars))

        # Coverage filter on full frame
        if not session_coverage(bars) and len(bars) < 200:
            log.debug("Skipping %s — sparse coverage", sym)
            continue
...
def cmd_train(args: argparse.Namespace, settings: Settings) -> None:
    ...
    log.info("Building training rows target=%.2f stop=%.2f H=%d", target, stop, horizon)
    rows = _sample_training_rows(settings, target_pct=target, stop_pct=stop, horizon=horizon)
    if len(rows) < 50:
        log.error(
            "Only %d training rows — run `python -m bot backfill` with Alpaca keys first. "
            "Generating synthetic demo rows for pipeline smoke-test.",
            len(rows),
        )
...
def _synthetic_rows(target: float, stop: float, horizon: int, n: int = 400) -> list[dict]:
    """Deterministic-ish synthetic data so train/backtest work offline without keys."""
    rng = np.random.default_rng(42)
    rows = []
    for i in range(n):
        side_long = i % 2 == 0
        # Feature correlated lightly with success
        mom = float(rng.normal(0, 0.01))
        vol = float(abs(rng.normal(0.2, 0.05)))
        # Higher momentum in direction → more success
...
def cmd_backtest(args: argparse.Namespace, settings: Settings) -> None:
    ...
    for r in rows:
        ...
        edge = expected_edge(p, target, stop, settings.fee_buffer_pct)
        pred_edges.append(edge)
        # Trade if edge positive
        if edge <= 0:
            continue
        ret = r["exit_return"]
        # Prefer labeled exit_return; for barrier success/fail approximate
    ...
    out = settings.data_dir / "exports" / "backtest_last.json"
...
def cmd_stream(args: argparse.Namespace, settings: Settings) -> None:
    if args.mode:
        # Override mode for this process without mutating cached settings incorrectly
        object.__setattr__(settings, "bot_mode", args.mode)
    engine = StreamEngine(settings)
    if args.once:
...
def build_parser() -> argparse.ArgumentParser:
    ...
    b = sub.add_parser("backfill", help="Download IEX minute bars (or yfinance daily fallback)")
    b.add_argument("--symbols", default="", help="Comma-separated symbols")
